use super::{command_text_for_source, sensor_index_for_source};
use crate::EditorWidget;
use lianli_shared::fonts::{font_label_for_path, DEFAULT_FONT_LABEL};
use lianli_shared::sensors::SensorInfo;
use lianli_shared::template::{FontRef, TextAlign, Widget, WidgetKind};
use slint::SharedString;

pub(in crate::editor) fn widget_to_editor(w: &Widget, sensors: &[SensorInfo]) -> EditorWidget {
...
pub(in crate::editor) fn font_ref_to_label(f: &FontRef) -> String {
    font_label_for_path(f.path.as_deref())
}

pub(in crate::editor) fn text_align_name(a: TextAlign) -> &'static str {
    match a {
        TextAlign::Left => "left",
...
pub(in crate::editor) fn blank_editor_widget() -> EditorWidget {
    EditorWidget {
