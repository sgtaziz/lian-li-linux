...
fn scale(color: [u8; 3], intensity: u8, brightness: u8) -> [u8; 3] {
    let brightness = [0u16, 64, 128, 192, 255][brightness.min(4) as usize];
    color.map(|channel| {
        ((((u16::from(channel) * u16::from(intensity)) >> 8) * brightness) >> 8) as u8
    })
}
...
pub(super) fn render(effect: &RgbEffect, fans: usize, bottom: bool) -> Result<Vec<Vec<[u8; 3]>>> {
    ...
    let led_count = fans * 26;
    let selected_half = usize::from(bottom);
    let mut frames = vec![vec![[0; 3]; led_count]; 200];

    for &(led, start) in PULSE_STARTS {
        if led >= led_count || led % 26 / 13 != selected_half {
            continue;
        }
        let color = effect
            .colors
        ...
        for (offset, intensity) in PULSE.into_iter().enumerate() {
            ...
            if (0..200).contains(&frame) {
                frames[frame as usize][led] = scale(color, intensity, effect.brightness);
            }
        }
    }
    ...
    for &(led, frame) in SINGLE_SPARKS {
        if led < led_count && led % 26 / 13 == selected_half {
            let color = effect
                ...
                .copied()
                .unwrap_or([0; 3]);
            frames[frame][led] = scale(color, 5, effect.brightness);
        }
    }
